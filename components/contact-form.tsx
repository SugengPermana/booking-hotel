...
import { ContactMessage,ContactActionState } from "@/lib/actions"
import clsx from "clsx";

const ContactForm = () => {
  const [state, formAction, isPending] = useActionState<ContactActionState, FormData>(ContactMessage,null);
  return (
      ...
      <form action={formAction}>
        <div className="grid md:grid-cols-2 gap-7 mt-6">
          <div>
            ...
            <div>
            <input type="email" name="email" className="bg-gray-50 p-3 border border-gray-200 rounded-sm w-full font-light" placeholder="YourName@gmail.com" />
            <div aria-live="polite" aria-atomic="true">
            ...
            <div aria-live="polite" aria-atomic="true">
              <p className="text-sm text-red-500 mt-2">{state?.error?.subject?.[0]}</p>
              </div>
            </div>
            <div className="md:col-span-2">
            ...
            <div aria-live="polite" aria-atomic="true">
              <p className="text-sm text-red-500 mt-2">{state?.error?.message?.[0]}</p>
              </div>
            </div>
          </div>
          <button type="submit" 
